...
TOKEN = '8867047734:AAHobvHxRe8_kUEiexuUVBf5dkHMLDCyknE'
OWNER_ID =7313156570
ADMIN_ID =7313156570
YOUR_USERNAME = '@YAMDHUD'
UPDATE_CHANNEL = 'https://t.me/NomanTechBot'
...
# ✅ NEW: File approval status constants
...
# ✅ NEW: Send file for approval function
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    # ✅ NEW: Check if file is approved before running
...
def create_main_menu_inline(user_id):
    ...
    if user_id in admin_ids:
        # ✅ NEW: Add pending files count for admins
        ...
        markup.add(buttons[3], admin_buttons[0])  # Pending files button
...
# ✅ NEW: Show pending files for admin
def _logic_view_pending(message):
    ...
    for idx, (user_id_file, file_name, file_type, uploaded_time) in enumerate(pending_files[:20], 1):  # Limit to 20
        try:
            uploaded_dt = datetime.fromisoformat(uploaded_time)
            time_ago = datetime.now() - uploaded_dt
            minutes = int(time_ago.total_seconds() / 60)
...
# ✅ NEW: Add pending files command for admins
...
# ✅ NEW: Approval callback handlers
def handle_approve_callback(call):
    """Handle approve file callback"""
    try:
        ...
        # Extract user_id and file_name from callback data
...
def handle_review_callback(call):
    """Handle review file callback (from pending list)"""
    try:
        admin_id = call.from_user.id
        if admin_id not in admin_ids:
            bot.answer_callback_query(call.id, "Admin permissions required.", show_alert=True)
        ...
        # Create review message with approve/reject buttons
        review_text = (
            f"📋 **File Review**\n\n"
            f"👤 **User ID:** `{user_id}`\n"
            f"📁 **File:** `{file_name}`\n"
...
def delete_bot_callback(call):
    try:
        ...
        # ✅ NEW: Also remove from approvals table
        with DB_LOCK:
            conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
            c = conn.cursor()
            try:
